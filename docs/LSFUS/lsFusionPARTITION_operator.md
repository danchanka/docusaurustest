...
The **PARTITION** operator creates a [property](lsFusionProperties.md) that implements [partition/order](lsFusionPartitioning_sorting_PARTITION_..._ORDER_.md) or [simple distribution](lsFusionDistribution_UNGROUP_.md).
...
-   calculates an [aggregate function](lsFusionSet_operations.md) for each object collection. In case of partition/order,
...
An [expression](lsFusionExpression.md) whose value is passed as an input to the aggregating function as an operand in case of partition/order. In case of distribution with type **PROPORTION** it defines the proportion, and with type **LIMIT** it defines the limit.
