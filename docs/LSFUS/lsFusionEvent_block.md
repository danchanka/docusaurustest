...
The event block of the  [FORM instruction](lsFusionFORM_instruction.md) - a set of constructions controlling [events](lsFusionForm_events.md) in an interactive form view.
...
The event block allows to define handlers for [form events](lsFusionForm_events.md) that occur as the result of certain user actions. Each block can have an arbitrary number of comma-separated event handlers. If several handlers are defined for an event, they are guaranteed to be executed in the order they are defined. 
