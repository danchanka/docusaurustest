...
The base element in **lsFusion** is the object. Each object is an *instance* of some *class*, which determines the qualities of all its instances. Classes in turn can be divided into [built-in classes](lsFusionBuilt-in_classes.md), which are responsible for primitive data types, and [user classes](lsFusionUser_classes.md). 
