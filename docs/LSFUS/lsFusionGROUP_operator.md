...
The **GROUP** operator creates a [property](lsFusionProperties.md) implementing [grouping](lsFusionGrouping_GROUP_.md).
...
For **AGGR** and **NAGGR** using this block explicitly (and not, say, an [**IF** operator](lsFusionIF_operator.md) in **GROUP** and **BY** blocks) only makes sense from the perspective of being able to change the created property to non-**NULL** in some automatic mechanisms of the platform (for example, [automatic resolution](lsFusionSimple_constraints.md) of simple constraints).
