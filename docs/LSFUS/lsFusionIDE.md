...
Select project type **lsFusion**. Make sure that the JDK is set and the link to the library with the  **lsFusion** is selected (when [automatic installation](lsFusionDevelopment_auto_.md) is used, this link is set automatically; otherwise, click *Download* or select the previously downloaded library *Create*) and then** **click *Next*.
...
To add a new [module](lsFusionModules.md), right-click the corresponding folder and select *New */ *lsFusion* from the menu:
...
If the platform is embedded to an existing project (for example, [via Maven](57738081.html#Development(manual)-maven) for a Maven project), you may need to manually create a server startup configuration for the applications. This is done as follows:
...
To compile a single JAR file that contains both the developed code and the application server itself, you can use the following [guide](https://blog.jetbrains.com/idea/2010/08/quickly-create-jar-artifact/). The result JAR file (*artifact*) can be used to install applications on a production server as described in the section [installing an applications server as a service](57738083.html#Execution(manual)-appservice). Note that since all modules and the server itself will be inside the same JAR file, the installation process is slightly different:
