...
The property and action [block of the FORM instruction](lsFusionFORM_instruction.md) adds [properties and actions](Form-structure_1573069.html#Formstructure-properties) to the [form structure](lsFusionForm_structure.md). 
...
In all [expressions](lsFusionExpression.md) and [context-dependent action operators](Action-operator_36307157.html#Actionoperator-contextdependent), you can use the names of the objects already declared on the form as parameters.
...
[Expression](lsFusionExpression.md) being added to the form structure.
...
Specifying a property that determines the footer of the property (action) being added. Used only in the [print view](lsFusionPrint_view.md) of the form.
...
If not specified, the corresponding option from the [property options](lsFusionProperty_options.md) is used. If it is also not specified, then the [default view type](Interactive-view_1573071.html#Interactiveview-defaultPropertyView) is used for the display group of this property or action.
...
Specifying that a property or an action should be added to the form structure before (keyword **BEFORE**) or after (keyword **AFTER**) the previously added property or action. Typically used in the [form extension](lsFusionForm_extension.md) concept.
...
Specifying the action that will be executed when the specified [form event](lsFusionForm_events.md) occurs.
...
Specifying the name to be used for [export/import](Structured-view_29884537.html#Structuredview-extid) operations of this property. Used only in the [structured](lsFusionStructured_view.md) view.
...
Specifying a [property/action group](lsFusionGroups_of_properties_and_actions.md) which the property (action) on the form belongs to. If this option is not specified, then the property group of the property itself will be used as the property (action) group on the form. 
