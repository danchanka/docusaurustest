...
In addition to the above operators, data input is also performed via the *dialog forms* of message display  [(ASK)](Show_message_MESSAGE_ASK.md#Showmessage(MESSAGE,ASK)-dialog)  and [form opening (DIALOG)](In_an_interactive_view_SHOW_DIALOG.md#Inaninteractiveview(SHOW,DIALOG)-dialog) operators in an interactive view.
